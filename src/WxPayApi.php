...
use qingsong\wxpayapi\WxPayException;
use qingsong\wxpayapi\WxPayConfig;
use qingsong\wxpayapi\Log;
...
class WxPayApi {
    ...
    public function __construct($options = array()) {
        ...
        self::$MCHID = $options['MCHID'];
        self::$SSLCERT_PATH = $options['SSLCERT_PATH'];
        self::$SSLKEY_PATH = $options['SSLKEY_PATH'];
    }

    //////发红包////////////////
    public static function tosendredpack($inputObj, $timeOut = 6) {
        $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack";

        $inputObj->Setwxappid(self::$APPID); //公众账号ID
        ...
        $response = self::postXmlCurlcert($xml, $url, true, $timeOut);

        $result = WxPayResults::Initredpack($response);
    ...
    //////企业付款给个人微信账号////////////////
    public static function tosendchange($inputObj, $timeOut = 6) {
//        $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack";
        $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";

        $inputObj->Setmchappid(self::$APPID); //公众账号ID
        $inputObj->SetMchid(self::$MCHID); //商户号
        //$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip	  
        //$inputObj->SetSpbill_create_ip("1.1.1.1");  	    
        $inputObj->SetNonce_str(self::getNonceStr()); //随机字符串
        //签名
    ...
    private static function postXmlCurlcert($xml, $url, $useCert = true, $second = 30) {
        $ch = curl_init();
        //设置超时
        curl_setopt($ch, CURLOPT_TIMEOUT, $second);
...
//            curl_setopt($ch, CURLOPT_CAINFO, WxPayConfig::SCAINFO_PATH);
        }
        //post提交方式
        curl_setopt($ch, CURLOPT_POST, TRUE);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
